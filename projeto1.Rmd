...
Além disso, as médias do estudo piloto para a variável **pressão sistólica** foram repassadas:

-   Grupo MN: 122,02

-   Grupo MP: 132.05

Logo o a diferença das médias aqui é: 

$\delta = |\mu_{MN} - \mu_{MP}| = |122,02 - 132,05| = $ `r abs(122.02 - 132.05)`

Portanto, se enquandra no caso imputado na tabela quando $\delta \approx 10$.
...
A fim de validar alguns pressupostos da regressão e a partir da *Figura 2* (contida no *apêndice*), sobre os resíduos, nota-se que a homocedasticidade é válida.
...
#plot(log2(novo_df$conc), novo_df$abs, type = "line")
...
previsao <- function(A){
  fA <- (1/slope.est) * (log( ((Amax.est - A)/(A - Amin))) + log2EC50.est*slope.est)
  return(fA)
}
...
nteo = 6
novo_df$fonte <- c(rep("Teorica",nteo), rep("Estimada",nrow(novo_df) - nteo))

novo_df %>% 
  ggplot(aes(x = log2conc, y = abs, color = fonte)) +
  geom_point(size = 3) +
  labs(x = "Concentração", y = "Absorbância", title = "Gráfico de adequação do modelo") +
  scale_fill_manual(values = c("A" = cor_roxa, "B" = cor_verde))
