...
resumo_testes <- function(teste, variavel){
    ...
    metodo <- teste$method  
    alpha <- 0.05
    conclusao <- if_else(pvalor >= alpha/2, "Não rejeita-se H0", "Rejeita-se H0")
...
Suposição do quiquadrado é freq > 5

```{r}
M = xtabs(~ grupo + escolaridade, data=df1)

xtabs(grupo, escolaridade, subset = df1)
M <- as.table(rbind(c(762, 327, 468), c(484,239,477)))
dimnames(M) <- list(Sexo=c("Controle","Tratamento"),
Partido=c("Democrat","Independent", "Republican"))
chisq.test(M)

fisher.test(M) # aqui da sem erro
...
hipo_categorica(df1, "grupo", "num_gestacoes")
```
...
#hipo_categorica(df2, "grupo", "quantos_anos_evento")
...
hipo_categorica(df2, "grupo", "tipo_parto")
```
...
hipo_categorica(df2, "grupo", "prematuro")
```

```{r}
# pode usar quiquadrado
tab <- xtabs(~ grupo + prematuro, df2_filtro)
chisq.test(tab)
```
...
```{r}
graficos(df3, "grupo", "pai1") 
realizar_teste(df3, "pai1")
```

### trombomodulina

```{r}
graficos(df3, "grupo", "trombomodulina") 
realizar_teste(df3, "trombomodulina")
```
...
```{r}
graficos(df_adma, "grupo", "adma")
realizar_teste(df_adma, "adma")
```

### aaat1

```{r}
graficos(df3, "grupo", "aaat1")
realizar_teste(df3, "aaat1")
```
...
```{r}
graficos(df_filtro, "grupo", "sflt1")
realizar_teste(df_filtro, "sflt1")
```
...
```{r}
graficos(df_filtro, "grupo", "dimero")
realizar_teste(df_filtro, "dimero")
```
