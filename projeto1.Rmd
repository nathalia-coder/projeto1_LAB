...
Além disso, as médias para a **pressão sistólica**, também do estudo piloto, são:
...
Averiguou-se qual seria a escolha do $\delta$ para o estudo, no caso: $\delta \approx 10$, logo esse valor será fixado como diferença entre as médias.

Então, escolhendo um nível de significância de $\alpha = 0,05$ e variandos-se o poder de teste de ($1 - \beta$), tem-se que os resultados encontrados estão na *Tabela 1*.

## 3.3 Curva de concentração do Sflt1
...
### 3.3.1 Estimação da concentração por imputação do limite inferior
...
## 3.5 Resumo testes de hipóteses
...
Optou-se por variar o $\beta$ no estudo (1 - $\beta$ indica o poder do teste). Testes mais poderosos implicam em maior rigor do tamanho amostral, espera-se aumento do tamanho da amostra quando avaliar-se isso. Vale lembrar que usualmente usa-se o poder de 90%.
...
tab_amostra %>% 
  kable(
    caption = "Cálculo do tamanho amostral para diferentes betas",
    format.args = list(big.mark = ".")) %>%
  kable_styling("striped", full_width = F)
```

## 4.2 Curva de concentração do Sflt1
...
Por meio da *Figura 1* é possível notar o comportamento da curva para os dados teóricos. Na *Figura 2*, tem-se as previsões para os dados abaixo da curva juntamente com os dados teóricos. A *tabela 5* consta todos resultados encontrados e foi deixado a referência repassada pela pesquisadora.
...
A fim de validar alguns pressupostos da regressão e a partir da *Figura 3* (contida no *apêndice*), sobre os resíduos, nota-se que a homocedasticidade é válida.
...
### 4.2.2 Estimação da concentração por imputação do limite inferior
...
funcao_tabela <- function(dados, grupo, variavel){
  return(
    ...
    adorn_rounding(4) %>%  adorn_pct_formatting(digits = 2) %>% 
      ...
      footnote(paste0("Variável de referência: ", variavel)) %>%
      kable_styling("striped", full_width = F)
  )
}
```
...
## 4.3 Resumo testes de hipóteses
...
tabelinha <- dados %>%
  kable(
    caption = "Probabilidade de desenvolver hipertensão",
    format.args = list(big.mark = ".")) %>%
  kable_styling("striped", full_width = F) %>%
...
Na análise, avaliamos uma amostra razoável e identificamos que o erro com $\delta$ menor que 10 é aceitável, fornecendo confiança em nossos resultados. Além disso, através da estimação dos parâmetros da curva logística de quatro parâmetros, e por imputação do limite inferior, foi possível a obtenção de valores da concentração de sFlt, resultados anteriormente inacessíveis por serem muito baixos.
