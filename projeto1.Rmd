...
A regressão logística é uma técnica de análise de dados que usa matemática para encontrar as relações entre dois ou mais fatores de dados. Em seguida, essa relação é usada para prever o valor de um desses fatores com base no(s) outro(s). A previsão geralmente tem um número finito de resultados, como sim ou não.

A regressão logística binária funciona bem para problemas de classificação binária que tenham apenas dois resultados possíveis. A variável dependente pode ter apenas dois valores, como sim e não ou 0 e 1. Nessa análise, nosso valor 0 corresponde a não ter hipertensão e o valor corresponde a ter.

No caso, múltiplas variáveis explicativas serão estudadas, para a correta avaliação de quais preveem a variável dependente, que é a hipertensão.

Os passos para selecionar as variáveis apropriadas para o modelo de regressão logística foram os seguintes: primeiramente, excluímos aquelas com alta proporção de valores faltantes. Em seguida, eliminamos as linhas com valores ausentes para as outras variáveis. 
Realizamos os testes avaliar diferença entre médias e associação das variáveis com hipertensão. Aquelas que apresentaram um p-valor menor ou igual a 0,25 entraram no nosso modelo inicial. O valor alto foi escolhido para uma pré seleção das variáveis, uma vez que seria muito difícil uma variável que apresentar um p-valor maior do que esse ter significância no modelo.

Uma observação importante é que criamos uma variável **hipertensão**, considerando pressões sistólica e diastólica acima de 140 e/ou 90, respectivamente, para classificar mulheres com hipertensão, independentemente das respostas do questionário.

Uma matriz de correlação exibe o grau de correlação entre várias interseções de medidas como uma matriz de células retangulares. Na matriz de dispersão, a ideia é a mesma, mas no lugar de um número, teremos um gráfico de correlação entre as variáveis em cada célula.

A multicolinearidade que consiste em um problema comum em regressões, no qual as variáveis independentes possuem relações lineares exatas ou aproximadamente exatas. Para avaliá-la, utilizamos uma matriz de dispersão e correlação e calculamos o VIF (fator de inflação da variância). Detectamos multicolinearidade significativa entre IMC, gordura, cintura e quadril. Isso pode afetar a significância dos coeficientes de regressão e, portanto, essas variáveis foram consideradas problemáticas. 
Também usamos o critério AIC (critério de informação de Akaike), que é uma métrica que mensura a qualidade de um modelo estatístico visando também a sua simplicidade. Após diversas iterações de modelos, confirmamos que a inclusão simultânea das variáveis problemáticas inflacionava o VIF. 

Nosso **modelo final** incluiu **grupo** e **IMC**. A inclusão de LDL foi feita em outro modelo, por ser teoricamente relevante na variável resposta. 
Por razão da multicolinearidade, não foi possível conciliar as variáveis IMC e gordura no mesmo modelo, mas como a segunda variável também é importante para a cliente, deixamos também, como pedido, outras opções de modelo. 
...
As colunas das tabelas geradas tabelas apresentam: Characteristic, que é nossa variável que está sendo avaliada, o OD (razão de chances), o 95% IC (intervalo de confiança de 95% para  a razão de chances) e p-value (p-valor). O valor 1 estar presente no intervalo de confiança corrobora para a afirmação de que a variável não é significativa.
...
modelo4<- glm(alta_pressao ~ grupo + gordura + ldl_dosado, data = dfmodelo, family = "binomial"(link = 'logit'))
summary(modelo4)
vif(modelo4)
exp(coef(modelo4))
exp(confint(modelo4))
...
As variáveis grupo e gordura são significativas para o modelo. O valor 1 não pertence ao intervalo do grupo, e é o limite inferior nas variáveis gordura e LDL).
Os hífens na linha do grupo 1 indicam que vamos comparar nosso grupo 2 (mulheres que tiveram pré-eclâmpsia) em relação a ele. A variável grupo é binária, então, nesse modelo, a mulher que está no grupo 2 tem um aumento de 116% nas chances de ser hipertensa, em relação a mulher que está no grupo 1 (mulheres que não tiveram pré-eclâmpsia). 
Podemos dizer que, para o aumento de uma unidade na gordura, esperamos ver um aumento de cerca de 4% nas chances da mulher ter hipertensão.
